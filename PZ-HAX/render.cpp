...
fb::AxisAlignedBox AABox_ATmine = { { -0.18, 0.000, -0.17, 0.0 }, { 0.185, 0.08,  0.155, 0.0 } };
fb::AxisAlignedBox AABox_Dynamite = { { -0.15, 0, -0.07 , 0.0 }, { 0.185, 0.1,  0.055, 0.0 } };
fb::AxisAlignedBox AABox_APmine = { { -0.06, 0.03, -0.05, 0.0 }, { 0.065, 0.23,  0.055, 0.0 } };
...
#pragma region utils draw
	...
	void DrawAABB(fb::AxisAlignedBox aabb, D3DXMATRIXA16 tranform, D3DXCOLOR color, float thickness)
	{
...
#pragma region utils
	...
	{
		ConfigList.clear();
		string ConfigDir = "C:\\BFV_HAX\\*.ini";
		CreateDirectoryW(L"C:\\BFV_HAX", NULL);
...
#pragma region menu utils
	...
	void aimbot()
	{
		...
		ImGui::BeginChild(XorStr("#Accuracy"), ImVec2(270, 175), true);
		{
			...
			ImGui::Checkbox(XorStr("AimBot knocked"), &g_Options.AIMBOT.aimatknocked);
	...
	void Visual()
	{
		...
		ImGui::Separator();
		if (ImGui::Button(XorStr("ESP PLAYERS"), ImVec2(100, 0)))
		{
		...
		ImGui::SameLine();
		if (ImGui::Button(XorStr("ESP LOOT"), ImVec2(100, 0)))
		{
			g_Options.MENU.iVisualsSubTab = 2;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("FONT & SIZE"), ImVec2(100, 0)))
		{
			g_Options.MENU.iVisualsSubTab = 3;
		}
		ImGui::Separator();
		...
		case 0:
		{
			ImGui::BeginChild(XorStr("#ESP_TEAM"), ImVec2(270, 150), true);
			{
				...
				ImGui::SameLine(150);
				ImGui::Checkbox(XorStr("ID"), &g_Options.ESP.ESP_TEAM.teamid);
				ImGui::Checkbox(XorStr("Knocked"), &g_Options.ESP.ESP_TEAM.teamKnocked);
			}ImGui::EndChild();
			ImGui::SameLine();
			ImGui::BeginChild(XorStr("#ESP_ENEMY"), ImVec2(270, 150), true);
			{
				...
				ImGui::SameLine(150);
				ImGui::Checkbox(XorStr("ID"), &g_Options.ESP.ESP_ENEMY.enemyid);
				ImGui::Checkbox(XorStr("Knocked"), &g_Options.ESP.ESP_ENEMY.enemyKnocked);
			}ImGui::EndChild();
			ImGui::BeginChild(XorStr("#ESP_OPTIONS"), ImVec2(270, 150), true);
			{
				...
				ImGui::Checkbox(XorStr("Color By ID"), &g_Options.ESP.colorbyid);
				ImGui::Checkbox(XorStr("OutLine"), &g_Options.ESP.esp_Outline);

			}ImGui::EndChild();
			ImGui::SameLine();
			ImGui::BeginChild(XorStr("#ESP_Battle"), ImVec2(270, 150), true);
			{
				ImGui::Text(XorStr("Other ESP"));
				ImGui::Separator();
				ImGui::Checkbox(XorStr("Ammo & Health Box"), &g_Options.ESP.Other.packsbox);
				ImGui::Checkbox(XorStr("Grenades"), &g_Options.ESP.Other.Grenade);
				ImGui::Checkbox(XorStr("ExplosionPacks"), &g_Options.ESP.Other.ExplosionPacks);
		...
		case 2:
		{
			ImGui::BeginChild(XorStr("#ESP_LOOT"), ImVec2(270, 150), true);
			{
				ImGui::Text(XorStr("ESP TEAM"));
				ImGui::Separator();
				ImGui::Checkbox(XorStr("Weapons"), &g_Options.ESP.LOOT.weapons);
				ImGui::SameLine(150);
				ImGui::Checkbox(XorStr("Ammo"), &g_Options.ESP.LOOT.ammo);
				ImGui::Checkbox(XorStr("Armor"), &g_Options.ESP.LOOT.armor);
				ImGui::SameLine(150);
				ImGui::Checkbox(XorStr("Health"), &g_Options.ESP.LOOT.health);
				ImGui::Checkbox(XorStr("Backpack"), &g_Options.ESP.LOOT.backpack);
	...
	void Colors()
	{
		if (ImGui::Button(XorStr("ESP Team"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 0;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("ESP Enemy"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 1;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("ESP Vehicle"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 2;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("ESP Radar"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 3;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("ESP ID"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 4;
		}
		ImGui::SameLine();
		if (ImGui::Button(XorStr("Other"), ImVec2(80, 0)))
		{
			g_Options.MENU.iColorsSubTab = 5;
		}
		ImGui::Separator();
		...
		case 5:
		{
			...
			ImVec2 lastCursor = ImGui::GetCursorPos();
			ImGui::ListBoxHeader("##0", ImVec2(240, 330));
			for (int i = 0; i < Globals::ColorsForPicker6.size(); i++)
			{
				bool selected = i == selectedItem;
				if (ImGui::Selectable(Globals::ColorsForPicker6[i].Name, selected))
					selectedItem = i;
			}
			ImGui::ListBoxFooter();
			ImGui::SameLine(0.0f, 15.0f);
			ImGui::BeginChild(XorStr("#generalcolors"), ImVec2(285, 330), false, ImGuiWindowFlags_NoScrollbar);
			{
				...
				ImGui::BeginChild(XorStr("#ColorsChild"), ImVec2(285, 125), true);
				{
					...
					ImGui::Spacing();
					ImGui::Spacing();
					ColorP col = Globals::ColorsForPicker6[selectedItem];
					int r = (col.Ccolor[0] * 255.f);
					int g = (col.Ccolor[1] * 255.f);
					int b = (col.Ccolor[2] * 255.f);
					ImGui::PushItemWidth(250);
	...
	void Settings()
	{
		...
		ImGui::BeginChild(XorStr("first child"), ImVec2(547, 120), true);
		{
			...
			{
				SaveSettings("C:\\BFV_HAX\\" + ConfigList[iConfigSelect]);
			}
			ImGui::SameLine();
			if (ImGui::Button(XorStr("Load Config")))
			{
				LoadSettings("C:\\BFV_HAX\\" + ConfigList[iConfigSelect]);
			}
			ImGui::Separator();
			...
			if (ImGui::Button(XorStr("Create  Config")))
			{
				...
				if (ConfigFileName != "") {
					SaveSettings("C:\\BFV_HAX\\" + ConfigFileName + ".ini");
					RefreshConfigs();
				}
			}
...
#pragma endregion
	fb::LinearTransform GetTransform(DWORD64 Entity)
	{
		...
		return m_pMem->Start(m_collection)->load<fb::LinearTransform>(ComponentCollectionOffset + 0x10);
	}
	void DrawModels()
	{
		...
		fb::ClientPlayer* LocalPlayer = OM->GetLocalPlayer();
		if (ValidPointer(LocalPlayer)) {
			fb::ClientSoldierEntity* pLocalSoldier = LocalPlayer->clientSoldierEntity.GetData();
			if (ValidPointer(pLocalSoldier)) {
				...
				else
				{
					for (int i = 0; i < g_HealthStations.size(); i++)
					{
						...
						fb::LinearTransform EntityTransform = GetTransform(ent);
						fb::AxisAlignedBox VehEnemyBox = m_pMem->Start(ent)->load<fb::AxisAlignedBox>(0x240);
						if(g_Options.ESP.Other.packsbox)
						DrawAABB(VehEnemyBox, EntityTransform, D3DXCOLOR(0, 0, 1, 1), 2);
					}
				...
				for (int i = 0; i < ents->size(); i++)
				{
					...
					if (entTeamId == LocalPlayer->teamId)
						continue;
					...
					if (g_Options.ESP.Other.ExplosionPacks)
					{
						if (exp_str.find("deployable_antitank_mine") != std::string::npos)
							DrawAABB(AABox_ATmine, EntityTransform, D3DXCOLOR(1, 0, 0, 1), 2);
						else if (exp_str.find("deployable_dynamite_sticky") != std::string::npos)
							DrawAABB(AABox_Dynamite, EntityTransform, D3DXCOLOR(1, 0, 0, 1), 2);
						else if (exp_str.find("deployable_smine") != std::string::npos)
							DrawAABB(AABox_APmine, EntityTransform, D3DXCOLOR(1, 0, 0, 1), 2);
						else if (exp_str.find("deployable_dynamite_sticky_projectile_pve_Mesh") != std::string::npos)
							DrawAABB(AABox_Dynamite, EntityTransform, D3DXCOLOR(1, 0, 0, 1), 2);
					}
				}
				...
				for (int i = 0; i < ents->size(); i++)
				{
					...
					fb::LinearTransform EntityTransform = GetTransform(ent);
					fb::AxisAlignedBox VehEnemyBox = m_pMem->Start(ent)->load<fb::AxisAlignedBox>(0x460);
					...
					if (teamid != LocalPlayer->teamId)
					{
						...
						{
							DrawAABB(VehEnemyBox, EntityTransform, D3DXCOLOR(1, 1, 0, 1), 2);
				...
				EM->assign_type(TYPEINFO_ClientProxyGrenadeEntity);
				ents = EM->get();

				for (int i = 0; i < ents->size(); i++)
				{
					DWORD64 ent = ents->at(i);
					BYTE entTeamId = m_pMem->Start(ent)->load<BYTE>(0x430);
					if (entTeamId == LocalPlayer->teamId)
						continue;
					fb::LinearTransform EntityTransform = GetTransform(ent);
					...
					m_pMem->Start(ent)->r(0x38)->r(0x100)->r(0x18)->Get<stringblob>(0x0, &exp_strb);
					std::string exp_str((const char*)&exp_strb);
					D3DXVECTOR3 Position = D3DXVECTOR3(EntityTransform._41, EntityTransform._42, EntityTransform._43);
					D3DXVECTOR3 PositionEnd = Position;
					PositionEnd.y += .2f;
					if (WorldToScreen(Position) && WorldToScreen(PositionEnd))
					{
						if (g_Options.ESP.Other.Grenade)
						{
							if (exp_str.find("fraggrenade_projectile_pve_Mesh") != std::string::npos);
							Renderer::GetInstance()->RenderText(font, "Fraggrenade", ImVec2(PositionEnd.x, PositionEnd.y), 14, D3DXCOLOR(1, 0, 0, 1));
						}
					}
				...
				EM->assign_type(TYPEINFO_ClientLootItemEntity);
				ents = EM->get();
				...
				for (int i = 0; i < ents->size(); i++)
				{
					...
					stringblob ItemName;
					UINT LootType;
					std::string loot_name((const char*)&LootName);
					std::string item_name((const char*)&ItemName);
					DWORD64 ent = ents->at(i);
					if (m_pMem->IsValid(ent) && ent != NULL)
						if ((m_pMem->Start(ent)->load<__int32>(0x238)) != -1)
						{

							fb::LinearTransform EntityTransform = GetTransform(ent);
							std::string exp_str((const char*)&LootName);

							m_pMem->Start(ent)->r(0x720)->Get<stringblob>(0x40, &LootName);
							m_pMem->Start(ent)->r(0x38)->Get<UINT>(0x118, &LootType);
							m_pMem->Start(ent)->r(0x38)->r(0x100)->r(0x0)->Get<stringblob>(0x18, &ItemName);

							if (loot_name != "" && item_name != "")
							{
								D3DXVECTOR3 Position = D3DXVECTOR3(EntityTransform._41, EntityTransform._42, EntityTransform._43);
								D3DXVECTOR3 PositionEnd = Position;
								PositionEnd.y += .2f;
								if (WorldToScreen(Position) && WorldToScreen(PositionEnd))
								{
									switch (LootType)
									{
									case fb::LootPickupType::LootPickupType_Weapon:
										if(g_Options.ESP.LOOT.weapons)
										Renderer::GetInstance()->RenderText(font, "Weapon", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 0, 0, 255));
										break;
									case fb::LootPickupType::LootPickupType_Ammo:
										if (g_Options.ESP.LOOT.ammo)
										Renderer::GetInstance()->RenderText(font, "Ammo", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 255, 0, 255));
										break;
									case fb::LootPickupType::LootPickupType_Health:
										if (g_Options.ESP.LOOT.health)
										Renderer::GetInstance()->RenderText(font, "Health", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(0, 255, 0, 255));
										break;
									case fb::LootPickupType::LootPickupType_Armor:
										if (g_Options.ESP.LOOT.armor)
										Renderer::GetInstance()->RenderText(font, "Armor", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 255, 255, 255));
										break;
									case fb::LootPickupType::LootPickupType_ArmorVest:
										if (g_Options.ESP.LOOT.armor)
										Renderer::GetInstance()->RenderText(font, "Armor Vest", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 255, 255, 255));
										break;
									case fb::LootPickupType::LootPickupType_Sidearm:
										if (g_Options.ESP.LOOT.armor)
										Renderer::GetInstance()->RenderText(font, "Side Arm", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 255, 255, 255));
										break;
									case fb::LootPickupType::LootPickupType_Backpack:
										if (g_Options.ESP.LOOT.backpack)
										Renderer::GetInstance()->RenderText(font, "Backpack", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(0, 0, 255, 255));
										break;
									default:
										break;
									}
				...
				EM->assign_type(TYPEINFO_ClientArmorVestLootItemEntity);
				ents = EM->get();
					...
					if (m_pMem->IsValid(ent) && ent != NULL)
						if ((m_pMem->Start(ent)->load<__int32>(0x238)) != -1)
						{
							...
							fb::LinearTransform EntityTransform = GetTransform(ent);					
								...
								{
									if(g_Options.ESP.LOOT.armor)
									Renderer::GetInstance()->RenderText(font, "Armor Vest", ImVec2(PositionEnd.x, PositionEnd.y), 18, D3DXCOLOR(255, 0, 0, 255));
	...
	fb::ClientPlayer* ClosestVisibleEnemy()
	{
		...
		fb::ClientPlayer* pLocalPlayer = OM->GetLocalPlayer();
		if (ValidPointer(pLocalPlayer)) {
				for (int i = 0; i < 64; i++) {
					fb::ClientPlayer* pPlayer = OM->GetPlayerById(i);
					if (ValidPointer(pPlayer))
					{

								std::vector<fb::ClientPlayer*> Players;
								Players.push_back(pPlayer);
								fb::ClientSoldierEntity* pSoldier = pPlayer->clientSoldierEntity.GetData();
														...
														for (size_t i = 0; i < Players.size(); i++) {
															...
															if (WorldToScreen(out)) {
																...
																if ((crossdis < closest)) {
																	...
																	{
																		if (g_Options.AIMBOT.aimatknocked) {
																			bClosestVisibleEnemy = Players[i];
																		}
																	}
																	closest = crossdis;
	...
	void Runaimbot()
	{
		...
		if (ValidPointer(player)) {
			fb::ClientSoldierEntity* aSoldier = player->clientSoldierEntity.GetData();
	...
	void DrawVisuals() {
		...
		if (ValidPointer(pGameRenderer))
		{
			...
			fb::ClientPlayer* pLocalPlayer = OM->GetLocalPlayer();
			if (ValidPointer(pLocalPlayer)) {
				fb::ClientSoldierEntity* pLocalSoldier = pLocalPlayer->clientSoldierEntity.GetData();
				if (ValidPointer(pLocalSoldier)) {
					for (int i = 0; i < 64; i++) {
						fb::ClientPlayer* pPlayer = OM->GetPlayerById(i);
						if (ValidPointer(pPlayer))
						{
							...
							if (ValidPointer(pSoldier)) {
								...
								if (ValidPointer(pHealthComponent)) {
									...
									D3DXCOLOR Teamknocked = D3DXCOLOR(g_Options.COLOR.Teamknocked[0], g_Options.COLOR.Teamknocked[1], g_Options.COLOR.Teamknocked[2], 255.f);
									D3DXCOLOR Visibleknocked = D3DXCOLOR(g_Options.COLOR.Visknocked[0], g_Options.COLOR.Visknocked[1], g_Options.COLOR.Visknocked[2], 255.f);
									D3DXCOLOR UnVisibleknocked = D3DXCOLOR(g_Options.COLOR.UnVisknocked[0], g_Options.COLOR.UnVisknocked[1], g_Options.COLOR.UnVisknocked[2], 255.f);
									...
									if (g_Options.ESP.colorbyid) {
										for (int id = 0; id < 64; id++)
										{
											if (pPlayer->teamId == id) {
												D3DXCOLOR EnemyColor = D3DXCOLOR(g_Options.COLOR.id[id].Team[0], g_Options.COLOR.id[id].Team[1], g_Options.COLOR.id[id].Team[2], 255.f);
												Playerbox = EnemyColor;
												Playerline = EnemyColor;
												PlayerSkeleton = EnemyColor;
												PlayerKnocked = EnemyColor;
												PlayerFiledBox = EnemyColor;
												PlayerDistance = EnemyColor;
												PlayerName = EnemyColor;
											}
										}
									...
									else
									{
										if (pPlayer->teamId == pLocalPlayer->teamId) {
											...
											if (pSoldier->IsVisible()) {
												...
												PlayerKnocked = UnVisibleknocked;
									...
									if (WorldToScreen(footLocation) && WorldToScreen(headLocation)) {
										...
										if (checkSetting(g_Options.ESP.ESP_TEAM.teamKnocked, g_Options.ESP.ESP_ENEMY.enemyKnocked)) {
											if (pHealthComponent->m_Health < 0)
												DrawSkeletonESP(pSoldier, PlayerKnocked);
										}
										...
										if (pHealthComponent->m_Health > 0) {
											...
											if (checkSetting(g_Options.ESP.ESP_TEAM.teamid, g_Options.ESP.ESP_ENEMY.enemyid)) {
												char txt[1024];
												sprintf(txt, "ID %s", std::to_string((int)pPlayer->teamId).c_str());
												texts.push_back(Text(txt, 2, g_Options.ESP.ESP_SETTINGS.espid, D3DXCOLOR(1.f, 1.f, 1.f, 1.f)));
											}
								...
								if (pPlayer->InVehicle())
								{
									...
									fb::ClientVehicleEntity* pVehicle = pPlayer->clientVehicleEntity.GetData();
									if (ValidPointer(pVehicle))
									{
										fb::HealthComponent* pVehicleGetHealthComponent = pVehicle->healthcomponent();
										if (ValidPointer(pVehicleGetHealthComponent)) {
											fb::VehicleEntityData* pVehicleData = pVehicle->vehicleEntityData();
												...
												if (ValidPointer(pVehicleData->m_NameSid)) {
													...
													if (checkSetting(g_Options.ESP.VEHICLE_TEAM.teamVehicle, g_Options.ESP.VEHICLE_ENEMY.enemyVehicle)) {
														if (g_Options.ESP.ESPVehicleMode == 0) {
															float perc = (pVehicleGetHealthComponent->m_VehicleHealth / pVehicleGetHealthComponent->m_VehicleMaxHealth);
															D3DXCOLOR HealthColor = D3DXCOLOR(1 - perc, perc, 0, 1);
															DrawAABB(TransformAABB.m_Box, TransformAABB.m_Transform, HealthColor, g_Options.ESP.ESP_SETTINGS.espvinbox);
														}
														else
														{
															DrawAABB(TransformAABB.m_Box, TransformAABB.m_Transform, VehicleColor, g_Options.ESP.ESP_SETTINGS.espvinbox);
														}
													}
													if (WorldToScreen(Position) && WorldToScreen(PositionEnd))
													{
														...
														if (checkSetting(g_Options.ESP.VEHICLE_TEAM.teamVehicledmg, g_Options.ESP.VEHICLE_ENEMY.enemyVehicledmg)) {
															Renderer::GetInstance()->HorizontalDrawHealth(X, PositionEnd.y - 10, W + 25, 3, pVehicleGetHealthComponent->m_VehicleHealth, pVehicleGetHealthComponent->m_VehicleMaxHealth, D3DXCOLOR(0, 1, 0, 1), D3DXCOLOR(1, 0, 0, 1));
														}
													}
												}
											}
